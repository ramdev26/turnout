...
if ($path === '/payhere/initiate' && $method === 'POST') {
  ...
  $requestBase = rtrim(payhere_request_base_url(), '/');
  if ($requestBase !== '') {
    $cfg['app_base_url'] = $requestBase;
    $cfg['notify_url'] = $requestBase . '/api/payhere/notify';
  }
  $merchantId = $cfg['merchant_id'];
...
function organizer_event_stats_map(PDO $pdo, array $eventIds): array {
  ...
  $ticketByEvent = [];
  $ticketStmt = $pdo->prepare(
    "SELECT event_id, COALESCE(SUM(quantity), 0) AS total_capacity
     FROM tickets
  ...
  foreach ($eventIds as $eventId) {
    $tickets = $ticketByEvent[$eventId] ?? ['totalCapacity' => 0];
    $attendees = $attendeeByEvent[$eventId] ?? ['attendeeTotal' => 0, 'checkedInCount' => 0];
    $map[(string)$eventId] = [
      'eventId' => (string)$eventId,
      'soldTickets' => (int)$attendees['attendeeTotal'],
...
function organizer_earnings_api_shape(PDO $pdo, int $organizerUserId): array {
  ensure_finance_tables($pdo);
  ensure_order_bank_transfer_columns($pdo);
  $totals = organizer_earnings_totals($pdo, $organizerUserId);
  ...
  $grossCents = (int)($totals['grossCents'] ?? 0);
  $feeCents = (int)($totals['feeCents'] ?? 0);
  $netCents = (int)($totals['netCents'] ?? 0);
...
if (preg_match('#^/events/(\\d+)/branding$#', $path, $m) && $method === 'POST') {
  ...
  foreach ([
    ...
    $raw = $body[$typeStyleKey];
    if ($raw === null || $raw === '') {
      unset($customization[$typeStyleKey]);
      continue;
    }
  ...
  $allowedTemplates = ['template-2', 'template-5', 'template-6', 'template-7', 'template-8', 'template-10', 'template-canvas'];
