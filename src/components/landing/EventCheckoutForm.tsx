...
type Props = {
  ...
  /** Optional back path for page layout (e.g. /e/slug or /events/id) */
  backHref?: string;
};

export const EventCheckoutForm: React.FC<Props> = ({
  event,
  orderItems,
  totalAmount,
  layout,
  ...
  // Reset policies / payment defaults when the form mounts (parent remounts modal via key).
  useEffect(() => {
  ...
  useEffect(() => {
    onPurchasingChange?.(isPurchasing);
  }, [isPurchasing, onPurchasingChange]);
  ...
  }, [orderItems, assignEachTicket]);

  useEffect(() => {
    if (checkoutFields.length < 1) return;
  ...
  }, [orderItems, checkoutFields.length]);
  ...
  const submitLabel = isPurchasing
    ? 'Processing…'
    : !prefillReady
      ? 'Preparing…'
      : totalAmount <= 0
        ...
        <div className="min-h-dvh w-full" style={pageSurfaceStyle}>
          <div className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 sm:py-8 lg:px-10 lg:py-10">
            {backHref ? (
              <Link
                to={backHref}
                className="inline-flex text-sm font-semibold underline-offset-2 hover:underline"
                style={{ color: 'var(--landing-text-muted)' }}
              >
                ← Back to event
              </Link>
            ...
            <form
              ...
              className="mt-6 grid gap-8 lg:mt-8 lg:grid-cols-[minmax(0,1fr)_20rem] lg:items-start xl:grid-cols-[minmax(0,1fr)_22rem]"
            >
              <div className="min-w-0 space-y-5">
                ...
                {termsBlock}
                {payError && (
                  <div className="rounded-xl border border-red-200 bg-red-50 p-2.5 text-xs font-medium text-red-700 lg:hidden">
                    {payError}
                  </div>
                )}
    ...
    <>
      ...
      >
        ...
        >
          ...
          <form onSubmit={handleSubmit(submitCheckout)} className="flex min-h-0 flex-1 flex-col">
            <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-5 py-4 sm:px-7 sm:py-6">
              {orderSummaryCard}
              <div className="mt-5 sm:mt-6">{formFields}</div>
            </div>
            ...
            >
              <div className="mb-2.5">{termsBlock}</div>
              {payError && (
